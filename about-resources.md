...
| ---------------|---------------|---------------|-----|-----|
| VPC Infrastructure Services | Virtual Private Cloud  | vpc | itself | Yes |
...
VPC Infrastructure Services uses an inherited model for access control. A role on the resource hierarchy, such as group, service, type, or instance, is inherited to the resources down the hierarchy. The access to a resource is determined by the resulting union of roles on the resource. The following diagram is a visual representation of the model.

![IAM model for VPC Infrastructure Services](images/vpc-iam.svg "IAM model for VPC Infrastructure Services"){: caption="Figure: IAM model for VPC Infrastructure Services" caption-side="bottom"}
...
The following table summarizes the roles required to perform specific actions on VPC Infrastructure Services resources.
